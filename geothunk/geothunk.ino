...
int mqttConnect() {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("Couldn't connect to MQTT; WiFi isn't connected");
    return 0;
  }
...
void setup() {
  ...
  if (configWasLoaded && WiFi.SSID()) {
    ...
    while (WiFi.status() != WL_CONNECTED) {
      // TODO - clear config if flash button is held
      if ((millis() % 500) < 100)
        digitalWrite(LED_PIN, HIGH);
      else
        digitalWrite(LED_PIN, LOW);
      delay(25); // we can't call status too fast
    }
    digitalWrite(LED_PIN, HIGH);
  } else {
    configureAP();
